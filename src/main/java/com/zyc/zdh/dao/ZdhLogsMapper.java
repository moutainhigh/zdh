...
public interface ZdhLogsMapper extends BaseMapper<ZdhLogs> {
    ...
    @Delete("delete from zdh_logs where etl_task_id = #{ids_str}")
    public int deleteBatchById(@Param("ids_str") String ids_str);

    @Select(value="select * from zdh_logs where etl_task_id=#{etl_task_id} and log_time >=#{start_time}  and log_time <=#{end_time}")
